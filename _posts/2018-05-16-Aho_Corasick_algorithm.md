...
#### $\text{POI }2000\text{ WIR}$

不显然，将病毒代码插入 $\text{AC}$ 自动机中后，若存在无限长的安全代码，则对其进行匹配，永远无法匹配成功，即 $\text{Trie}$ 图上存在一个由非病毒结点构成的环。需要注意的是，$\text{Fail}$ 指针指向病毒结点的结点同样需要被排除，因为病毒串也是这类结点对应串的子串。

因此，只需建立 $\text{AC}$ 自动机之后跑一遍 $\text{DFS}$ 找环即可。为避免重复访问，对访问过的边进行记忆化。
